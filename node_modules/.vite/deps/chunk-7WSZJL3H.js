...
import {
  ...
  validate2,
  verifyHash
} from "./chunk-6O7JQSWI.js";
import {
  secp256k1
} from "./chunk-NLSX6P6S.js";
import {
...
function getCreateAddress(opts) {
  const from4 = toBytes(getAddress(opts.from));
  let nonce = toBytes(opts.nonce);
  if (nonce[0] === 0)
    nonce = new Uint8Array([]);
  return getAddress(`0x${keccak256(toRlp([from4, nonce], "bytes")).slice(26)}`);
}
function getCreate2Address(opts) {
  ...
  return getAddress(slice(keccak256(concat([toBytes("0xff"), from4, salt, bytecodeHash])), 12));
}
...
async function getWebSocketRpcClient(url, options = {}) {
  ...
  return getSocketRpcClient({
    async getSocket({ onClose, onError, onOpen, onResponse }) {
      const WebSocket = await import("./native-E4SBCRKR.js").then((module) => module.WebSocket);
...
async function verifyHash2({ address, hash, signature }) {
...
async function getTransaction(client, { blockHash, blockNumber, blockTag: blockTag_, hash, index: index2, sender, nonce }) {
  var _a, _b, _c;
  const blockTag = blockTag_ || "latest";
  const blockNumberHex = blockNumber !== void 0 ? numberToHex(blockNumber) : void 0;
...
async function simulateCalls(client, parameters) {
  ...
  const getBalanceData = account ? encode3(from2("constructor(bytes, bytes)"), {
...
async function sendUnsignedTransaction(client, args) {
  var _a, _b, _c;
  const { accessList, data, from: from4, gas, gasPrice, maxFeePerGas, maxPriorityFeePerGas, nonce, to, value, ...rest } = args;
  const chainFormat = (_c = (_b = (_a = client.chain) == null ? void 0 : _a.formatters) == null ? void 0 : _b.transactionRequest) == null ? void 0 : _c.format;
  const format = chainFormat || formatTransactionRequest;
  const request = format({
    // Pick out extra data that might exist on the chain's transaction request type.
...
//# sourceMappingURL=chunk-7WSZJL3H.js.map
